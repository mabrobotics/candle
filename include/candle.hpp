...
namespace mab
{
	...
	/*! \class Candle
		...
		This class is an connector between non-realtime user code and real-time CANdle firmware. It
	   can be used to configure MD80's via FDCAN, as well as control them. It can automatically
	   communicate with CANdle/MD80s to relieve the user from requiring them to manually control USB
	   and FDCAN communications.
	*/
	class Candle
	{
	  public:
		 ...
		 * @return A functional CANdle class object if succesfull, a nullptr if critical failure
		 * occured.
		 */
		explicit Candle(CANdleBaudrate_E  canBaudrate,
		...
		/**
		 * @brief A destructor of Candle class. Takes care of all started threads that need to be
		 * stopped before clean exit
		 */
		~Candle();
		...
		*/
		...
		/**
		 * @brief Sets transmit thread sleep time (can be used to free resources when highest
		 * communication frequency is not needed)
		 */
		...
		*/
		...
		@param canBaudrate enum listing all available baudrates. CAN_BAUD_1M is equal to baudrate of
		...
		*/
		...
		@param newTimeout FDCAN watchdog timeout in milliseconds. If set to 0 the watchdog is
		disabled.
		@return true if all parameters were changed succesfully, false otherwise
		*/
		bool configMd80Can(uint16_t			canId,
						   uint16_t			newId,
		...
		*/
		...
		/**
		@brief Searched if the drive with provided FDCAN canId exists in `Md80s` list (exists only
		if was previously added by `addMd80` method)
		@param canId ID of the drive
		...
		*/
		...
		/**
		@brief Begins auto update mode. In this mode, host and CANdle will automatically exchange
		USB messages with md80 commands and states. In this mode CANdle will automatically send
		commands and gather state from all MD80's added to update vector with `::addMd80` method. In
		this mode no config* or control* methods can be called.
		@return true if mode was set succesfully, false otherwise
		*/
		bool begin();
		/**
		@brief Ends auto update mode. Sets mode back to idle (config) mode. In this mode, control*,
		config* and other methods can be used.
		@return true if mode was set succesfully, false otherwise
		*/
		bool end();
		/**
		@brief Clears all parameters set on the CANdle, returning it to the state same as after a
		powerup.
