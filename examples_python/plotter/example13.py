...
layout = [[sg.Text("Motor Output")],
            [sg.Combo(['Velocity Mode', 'Position Mode'], default_value='Velocity Mode', key='-MODE-', enable_events=True)],
    ...
    while True:
        ...
        if event == '-MODE-': # If user changes mode, load PID registers for apropriate PID controller
            ...
            if values['-MODE-'] == 'Position Mode':
                window['-KP-'].update(str(candle.readMd80Register(ids[0],pyCandle.Md80Reg_E.motorPosPidKp, reg.RW.positionPidGains.kp)))
                window['-KI-'].update(str(candle.readMd80Register(ids[0],pyCandle.Md80Reg_E.motorPosPidKi, reg.RW.positionPidGains.ki)))
                window['-KD-'].update(str(candle.readMd80Register(ids[0],pyCandle.Md80Reg_E.motorPosPidKd, reg.RW.positionPidGains.kd)))
                window['-WINDUP-'].update(str(candle.readMd80Register(ids[0],pyCandle.Md80Reg_E.motorPosPidWindup, reg.RW.positionPidGains.intWindup)))
                window['-MAXTORQUE-'].update(str(candle.readMd80Register(ids[0],pyCandle.Md80Reg_E.motorVelPidOutMax, reg.RW.velocityPidGains.outMax)))
                window['-MAXVEL-'].update(str(candle.readMd80Register(ids[0],pyCandle.Md80Reg_E.motorPosPidOutMax, reg.RW.positionPidGains.outMax)))
...
async def Run(mode='Velocity Mode', setpoint=VELOCITY_SETPOINT, showVelocity=True, showPosition=False,
    ...
    Run.lp = LivePlot(xlim=(0,timeOfTest),ylim=(0,setpoint*1.5)) # Create a live plot with set dimensions (nessecary for live plotting)
    ...
    while(time.time() - startTime < timeOfTest):
        ...
        # Append data to plot
        if(showVelocity):
            Run.lp.append_data("Velocity", timeStamp, float(candle.md80s[0].getVelocity()))
        if(showPosition):
            Run.lp.append_data("Position", timeStamp, float(candle.md80s[0].getPosition()))
        if(showTorque):
            Run.lp.append_data("Torque", timeStamp, float(candle.md80s[0].getTorque()))
        if(showTemperature):
            Run.lp.append_data("Temperature", timeStamp, float(candle.md80s[0].getTemperature()))
